...
class ContextObject:
    ...
    def _dendro_set_attribute_where_name_may_have_dots(self, name, value):
        if '.' not in name:
...
def _run_job_child_process(*, job_id: str, job_private_key: str, processors: List[AppProcessor]):
    ...
    print(f'[dendro] Processor: {app_name}:{processor_name}')
    ...
    for input in processor._inputs:
        if not input.list:
            ...
            context._dendro_set_attribute(input.name, x)
        else:
    ...
    for parameter in processor._parameters:
        ...
        context._dendro_set_attribute_where_name_may_have_dots(parameter.name, parameter_value)
