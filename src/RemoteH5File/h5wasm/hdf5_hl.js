import ModuleFactory from "./hdf5_util_jfm.js";
export var Module; //: H5WasmModule = null;
export var FS = null;
const ready = ModuleFactory({ noInitialRun: true }).then((result) => {
  Module = result;
  FS = Module.FS;
  return Module;
});
...
function process_data(data, metadata, json_compatible = false) {
  ...
  if (type === Module.H5T_class_t.H5T_STRING.value) {
    if (metadata.vlen) {
      let output = [];
      let reader =
        metadata.cset == 1 ? Module.UTF8ToString : Module.AsciiToString;
    ...
    type === Module.H5T_class_t.H5T_INTEGER.value ||
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let member of compound_type.members) {
        let member_data = row_data.slice(
          member.offset,
  ...
  if (
    known_type &&
    (Array.isArray(output_data) || ArrayBuffer.isView(output_data)) &&
    !shape?.length
  ) {
...
function isH5PYBooleanEnum(enum_type) {
  return (
    Object.keys(enum_type.members).length === 2 &&
...
function prepare_data(data, metadata, shape, maxshape) {
  ...
  if (maxshape === undefined || maxshape === null) {
    ...
    final_maxshape = maxshape.map((dim) =>
      dim === null ? H5S_UNLIMITED : BigInt(dim),
    );
  }
  data = Array.isArray(data) || ArrayBuffer.isView(data) ? data : [data];
  let total_size = Number(
    final_shape.reduce((previous, current) => current * previous, 1n),
  );
  ...
  if (metadata.type === Module.H5T_class_t.H5T_STRING.value) {
    ...
    throw new Error(
      `data with type ${metadata.type} can not be prepared for write`,
    );
  }
...
const int_fmts = new Map([
  [1, "b"],
  [2, "h"],
  [4, "i"],
  [8, "q"],
]);
const float_fmts = new Map([
  [2, "e"],
  [4, "f"],
  [8, "d"],
]);
...
function metadata_to_dtype(metadata) {
  const { type, size, littleEndian, signed, compound_type, array_type, vlen } =
    metadata;
  if (type == Module.H5T_class_t.H5T_STRING.value) {
...
class HasAttrs {
  ...
  create_attribute(name, data, shape, dtype) {
    ...
    if (!metadata.littleEndian) {
      throw new Error(
        "create_attribute with big-endian dtype is not supported",
      );
    }
    const { data: prepared_data, shape: guessed_shape } = prepare_data(
      data,
      metadata,
      shape,
      shape,
...
export class Group extends HasAttrs {
  ...
  *items() {
  ...
  create_dataset(name, data, shape, dtype, maxshape, chunks) {
    ...
    } = prepare_data(data, metadata, shape, maxshape);
    const final_shape = shape?.map(BigInt) ?? guessed_shape;
  ...
  create_external_link(file_name, target, name) {
    // create a soft link in this group named *name* to *target* (absolute path)
    const link_name = this.path + "/" + name;
    return Module.create_external_link(
      this.file_id,
      file_name,
      target,
      link_name,
...
export class File extends Group {
  constructor(filename, mode = "r") {
    ...
    if (["H5F_ACC_TRUNC", "H5F_ACC_EXCL"].includes(access_mode)) {
      file_id = Module.ccall(
        "H5Fcreate",
        "bigint",
        ["string", "number", "bigint", "bigint"],
        [filename, h5_mode, 0n, 0n],
      );
    } else {
...
export class Dataset extends HasAttrs {
  ...
  slice(ranges) {
    ...
    let count = shape.map((s, i) =>
      BigInt(
        Math.min(s, ranges?.[i]?.[1] ?? s) - Math.max(0, ranges?.[i]?.[0] ?? 0),
      ),
    );
    let offset = shape.map((s, i) =>
      BigInt(Math.min(s, Math.max(0, ranges?.[i]?.[0] ?? 0))),
    );
    let total_size = count.reduce(
  ...
  resize(new_shape) {
    const result = Module.resize_dataset(
      this.file_id,
      this.path,
      new_shape.map(BigInt),
    );
...
function create_nested_array(value, shape) {
  ...
  const dims_product = shape.reduce(
    (previous, current) => previous * current,
    1,
  );
  if (total_length !== dims_product) {
    console.warn(
      `shape product: ${dims_product} does not match length of flattened array: ${total_length}`,
    );
  }
  // Get reshaped output:
  let output = value;
