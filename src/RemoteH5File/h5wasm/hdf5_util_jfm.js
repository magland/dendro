...
var Module = (() => {
  ...
  return function (Module) {
      ...
      createLazyFile: (
        ...
        const smartDetermineChunkRangeToLoad = (chunkNum, datalength) => {
          if (chunkNum === smartLastChunkNum + 1) {
            ...
            // enforce a maximum
            smartNumChunksInString = Math.min(
              smartNumChunksInString,
              Math.ceil((15 * 1024 * 1024) / chunkSize),
            );
          } else {
        ...
        LazyUint8Array.prototype.cacheLength =
          function LazyUint8Array_cacheLength() {
            ...
            var header;
            var hasByteServing =
              (header = headResponseHeaders["Accept-Ranges"]) &&
              header === "bytes";
            var usesGzip =
              (header = headResponseHeaders["Content-Encoding"]) &&
              header === "gzip";
            if (!hasByteServing) {
            ...
            lazyArray.setDataGetter((chunkNum) => {
              ...
              if (typeof lazyArray.chunks[chunkNum] == "undefined") {
                const chunkRangeToLoad = smartDetermineChunkRangeToLoad(
                  chunkNum,
                  datalength,
                );
                ...
                if (chunkRangeToLoad[1] - chunkRangeToLoad[0] === 1) {
                  ...
                  for (
                    let num = chunkRangeToLoad[0];
                    num < chunkRangeToLoad[1];
                    num++
                  ) {
                    lazyArray.chunks[num] = concatenatedData.subarray(
                      (num - chunkRangeToLoad[0]) * chunkSize,
                      (num - chunkRangeToLoad[0] + 1) * chunkSize,
                    );
                  }
                }
    ...
    var _H5Fcreate = (Module["_H5Fcreate"] = function () {
      return (_H5Fcreate = Module["_H5Fcreate"] =
        Module["asm"]["H5Fcreate"]).apply(null, arguments);
    });
    var _H5Fopen = (Module["_H5Fopen"] = function () {
      return (_H5Fopen = Module["_H5Fopen"] = Module["asm"]["H5Fopen"]).apply(
        null,
        arguments,
      );
    });
    var _H5Fclose = (Module["_H5Fclose"] = function () {
      return (_H5Fclose = Module["_H5Fclose"] =
        Module["asm"]["H5Fclose"]).apply(null, arguments);
    });
