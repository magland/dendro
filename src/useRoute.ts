...
const useRoute = () => {
  ...
  const route: Route = useMemo(() => {
    if (p === "/services") {
      ...
      const jobDefinition = jobDefinitionJson
        ? JSON.parse(decodeURIComponent(jobDefinitionJson))
        : undefined;
  ...
  const setRoute = useCallback(
    (r: Route) => {
      if (r.page === "home") {
        ...
        navigate(
          `/register_compute_client/${r.serviceName}/${r.computeClientName}`,
        );
      } else if (r.page === "job") {
        ...
        if (r.jobDefinition) {
          qstrs.push(
            `job_definition=${encodeURIComponent(JSON.stringify(r.jobDefinition))}`,
          );
        }
        if (r.jobId) {
