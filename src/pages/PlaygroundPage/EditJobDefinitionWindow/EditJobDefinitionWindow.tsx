...
import { PairioAppProcessor, PairioAppProcessorInputFile, PairioAppProcessorParameter, PairioJobDefinition } from "../../../types";
...
const EditJobDefinitionWindow: FunctionComponent<EditJobDefinitionWindowProps> = ({jobDefinition, setJobDefinition, processor, readOnly, show='all', setValid}) => {
    const setParameterValue = useCallback((name: string, value: any) => {
        setJobDefinition(pairioJobDefinitionReducer(jobDefinition, {
            type: 'setParameter',
            name,
            value
        }))
    ...
    const rows = useMemo(() => {
        ...
        nodes.forEach(node => {
            ...
            if (node.type === 'group') {
                ...
                if (node.fieldType === 'input') {
                    ...
                    const value = jobDefinition?.inputFiles.find(f => (f.name === node.name))?.url
                    const inputFile = processor.inputs.find(i => (i.name === node.name))
                    if (!inputFile) {
                        console.warn(`Unexpected: processor input file not found: ${node.name}`)
                        return
                    }
                    ret.push(<InputRow
                        key={node.name}
                    ...
                    const value = jobDefinition?.parameters.find(f => (f.name === node.name))?.value
        ...
        return ret
    }, [expandedGroups, jobDefinition, nodes, processor.inputs, processor.outputs, processor.parameters, readOnly, setParameterValue, validParametersDispatch, setJobDefinition, show])
    return (
        <div>
...
const InputRow: FunctionComponent<InputRowProps> = ({inputFile, url, setUrl, setValid, readOnly, visible}) => {
...
const OutputRow: FunctionComponent<OutputRowProps> = ({name, description, visible}) => {
    return (
        <tr>
...
const ParameterRow: FunctionComponent<ParameterRowProps> = ({parameter, value, setValue, setValid, readOnly, secret, visible}) => {
...
const EditParameterValue: FunctionComponent<EditParameterValueProps> = ({parameter, value, setValue, setValid}) => {
...
const EditUrl: FunctionComponent<EditUrlProps> = ({url, setUrl, setValid}) => {
    useEffect(() => {
        if (!url) setValid(false)
        setValid(true)
    }, [setValid, url])
    return (
        <input type="text" value={url} onChange={evt => {setUrl(evt.target.value)}} style={{width: 250}} />
    )
}
