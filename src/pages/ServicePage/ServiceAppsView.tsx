...
export const ServiceAppsView: FunctionComponent<ServiceAppsViewProps> = ({
  serviceName,
}) => {
  ...
  const handleAddServiceApp = useCallback(async () => {
    const serviceAppSourceUri = prompt(
      "Enter the source URI for the new service app",
    );
    if (!serviceAppSourceUri) return;
    const serviceApp = await addServiceAppFromSourceUri({
      sourceUri: serviceAppSourceUri,
    });
  ...
  return (
    <div>
      ...
      <ServiceAppsTable serviceApps={serviceApps} />
    </div>
  );
};
